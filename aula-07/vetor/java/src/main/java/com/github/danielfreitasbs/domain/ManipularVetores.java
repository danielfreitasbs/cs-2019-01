...
public final class ManipularVetores {
  ...
  public static void letrasRepetidas(final char[] cadeiaCaracteres) {
    ...
    validacaoVetorChar(cadeiaCaracteres);
  ...
  /**
   * Realiza a impressão de todos os valores e quantidades do numero que estão mapeados no map
   * ocorrencias.
   * 
  ...
  private static void imprimirOcorrenciasAleatorias(HashMap<Integer, Integer> ocorrencias) {
    ...
    ocorrencias.forEach((valor, ocorrencia) -> {
      linhaImpressao.append("Valor: ").append(valor).append(", Ocorrencias: ").append(ocorrencia);
  ...
  /**
   * Realizada a contabilidade de quantas vezes existe o numero dentro de uma lista. após, a
   * contabilidade é adicionado o valor e quantidade de vezes que ele é encontrado na lista.
   * 
   ...
   */
  private static void calculaOcorrencias(HashMap<Integer, Integer> ocorrencias,
  ...
  private static void sorteiaNumero(List<Integer> numerosEncontrados) {
    ...
    for (int iterador = 0; iterador <= quantidadeSorteios; iterador++) {
      int numeroSorteado =
          sorteador.ints(limiteMinimoIntervalo, (limiteMaximoIntervalo + 1)).findFirst().getAsInt();
  ...
  static void validacaoVetorChar(final char[] vetor) {
    final int limiteSuperiorVetor = 1;
    if (vetor.length <= limiteSuperiorVetor) {
      throw new ArrayStoreException("Não existem valores nos vetores");
    }
